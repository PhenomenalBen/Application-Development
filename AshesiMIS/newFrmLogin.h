    ...
    private: System::Void btnLogin_Click(System::Object^ sender, System::EventArgs^ e) {
        ...
        try {
            ...
            if (sqlDR->Read()) {
                ...
                MessageBox::Show(String::Format(L"Hello {0}", sqlDR["FirstName"]->ToString()), L"", MessageBoxButtons::OK);
                this->Close();
            }
            else {
                MessageBox::Show(L"Email or Password is incorrect", L"", MessageBoxButtons::OK);
                return;
            }
        }
        catch (Exception^ ex) {
            MessageBox::Show(String::Format(L"Failed to connect to database: {0}", ex->Message),
                L"Data Base Error Connection", MessageBoxButtons::OK);
        }
        finally {
